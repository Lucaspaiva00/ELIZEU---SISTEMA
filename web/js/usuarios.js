...
const PERFIS_USUARIO = {
    ADMIN: { nome: "Administrador", classe: "perfil-admin", descricao: "Acesso completo, inclusive empresa e usuários." },
    GERENTE: { nome: "Gerente", classe: "perfil-gerente", descricao: "Gestão das rotinas operacionais e relatórios." },
    VENDEDOR: { nome: "Vendedor", classe: "perfil-vendedor", descricao: "Clientes, orçamentos e atividades comerciais." },
    FINANCEIRO: { nome: "Financeiro", classe: "perfil-financeiro", descricao: "Contas, movimentações, caixas e relatórios." },
    ESTOQUE: { nome: "Estoque", classe: "perfil-estoque", descricao: "Produtos, materiais e movimentações de estoque." },
    FISCAL: { nome: "Fiscal", classe: "perfil-fiscal", descricao: "Configuração da empresa e documentos fiscais." }
};
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.getElementById("usuarioPerfil").addEventListener("change", atualizarDescricaoPerfil);
...
async function carregarUsuarios() {
    ...
    if (!resposta?.sucesso) {
        document.getElementById("tabelaUsuarios").innerHTML = `<tr><td colspan="6" class="usuarios-empty usuarios-error"><i class="fas fa-lock"></i><strong>Acesso restrito</strong><span>${escapar(resposta?.mensagem || "Não foi possível carregar os usuários.")}</span></td></tr>`;
        return;
    }
...
function renderizarUsuarios() {
    ...
    if (!filtrados.length) {
        corpo.innerHTML = '<tr><td colspan="6" class="usuarios-empty"><i class="fas fa-users-slash"></i><strong>Nenhum usuário encontrado</strong><span>Ajuste os filtros ou cadastre um novo usuário.</span></td></tr>';
        return;
    }
    corpo.innerHTML = filtrados.map((item) => {
        ...
        return `<tr>
            ...
            <td data-label="Ações"><div class="usuario-actions"><button class="action-btn" type="button" onclick="editarUsuario(${item.id})" title="Editar"><i class="fas fa-pen"></i></button><button class="action-btn" type="button" onclick="abrirModalSenha(${item.id})" title="Redefinir senha"><i class="fas fa-key"></i></button><button class="action-btn ${item.ativo ? "action-danger" : "action-success"}" type="button" onclick="alternarStatusUsuario(${item.id})" title="${item.ativo ? "Bloquear" : "Ativar"}"><i class="fas fa-${item.ativo ? "user-lock" : "user-check"}"></i></button></div></td>
        </tr>`;
    }).join("");
}
...
async function salvarUsuario(evento) {
    ...
    const resposta = id ? await put(`/usuarios/${id}`, dados) : await post("/usuarios", dados);
    if (!resposta?.sucesso) return mostrarMensagem(resposta?.mensagem || "Erro ao salvar usuário.");
    fecharModalUsuario();
    await carregarUsuarios();
    mostrarMensagem(resposta.mensagem || "Usuário salvo com sucesso.");
}
...
async function alternarStatusUsuario(id) {
    ...
    if (!resposta?.sucesso) return mostrarMensagem(resposta?.mensagem || "Erro ao alterar o status.");
    await carregarUsuarios();
...
function fecharModalSenha() { document.getElementById("modalSenhaUsuario").classList.remove("active"); }

async function redefinirSenha(evento) {
    evento.preventDefault();
    const novaSenha = document.getElementById("novaSenhaUsuario").value;
    if (novaSenha !== document.getElementById("confirmarSenhaUsuario").value) return mostrarMensagem("As senhas informadas não coincidem.");
    const resposta = await patch(`/usuarios/${document.getElementById("senhaUsuarioId").value}/senha`, { novaSenha });
    if (!resposta?.sucesso) return mostrarMensagem(resposta?.mensagem || "Erro ao redefinir a senha.");
    fecharModalSenha();
    mostrarMensagem(resposta.mensagem);
}

function atualizarDescricaoPerfil() { document.getElementById("descricaoPerfil").textContent = PERFIS_USUARIO[document.getElementById("usuarioPerfil").value]?.descricao || ""; }
function alternarVisibilidade(id, botao) { const campo = document.getElementById(id); campo.type = campo.type === "password" ? "text" : "password"; botao.querySelector("i").className = `fas fa-${campo.type === "password" ? "eye" : "eye-slash"}`; }
function escapar(valor) { const elemento = document.createElement("div"); elemento.textContent = String(valor ?? ""); return elemento.innerHTML; }
function iniciais(nome) { return String(nome || "U").trim().split(/\s+/).slice(0, 2).map((parte) => parte[0]).join("").toUpperCase(); }
function formatarData(valor) { return valor ? new Date(valor).toLocaleDateString("pt-BR") : "—"; }
function formatarDataHora(valor) { return new Date(valor).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" }); }
