...
async function gerarPdfOrcamento(id) {
    ...
    // Os cadastros antigos podem trazer o bairro no final do logradouro.
    // O bairro digitado no campo próprio tem prioridade; não inferimos uma
    // localidade arbitrária de um endereço sem marcador reconhecível.
    const normalizarLocalidadePdf = (valor) => String(valor ?? "")
        .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
        .toLowerCase().replace(/[^a-z0-9]/g, " ").replace(/\s+/g, " ").trim();

    const separarEnderecoBairroPdf = (endereco, bairro, cidade) => {
        const original = String(endereco ?? "").trim();
        const bairroInformado = String(bairro ?? "").trim();
        if (!original) return { endereco: "", bairro: bairroInformado };

        const prefixoBairro = String.raw`(?:setor|st\.?|jardim|jd\.?|vila|residencial|res\.?|bairro|parque|pq\.?|conjunto|conj\.?|loteamento|ch[aá]cara)`;
        const padraoMarcado = new RegExp(`^(.+?)(?:\\s*[,;]\\s*|\\s+[-–—]\\s+)((${prefixoBairro})\\b.+)$`, "i");
        // Sem pontuação: "Rua A Qd 1 Lt 2 Setor Goiânia 2".
        const padraoSemSeparador = new RegExp(`^(.+?\\d)\\s+((?:${prefixoBairro})\\b.+)$`, "i");
        const candidato = original.match(padraoMarcado) || original.match(padraoSemSeparador);

        let rua = original;
        let bairroExtraido = "";
        if (candidato) {
            rua = candidato[1].trim().replace(/[\s,;\-–—]+$/, "");
            bairroExtraido = candidato[2].trim();
        }

        if (bairroInformado) {
            const bairroNormalizado = normalizarLocalidadePdf(bairroInformado);
            const extraidoNormalizado = normalizarLocalidadePdf(bairroExtraido);
            // Se os dois campos divergirem, usamos o bairro cadastrado, mas não
            // descartamos texto potencialmente importante do logradouro.
            const bairroComCidade = normalizarLocalidadePdf(
                `${bairroInformado} ${String(cidade || "").trim()}`
            );
            if (bairroExtraido && extraidoNormalizado !== bairroNormalizado &&
                extraidoNormalizado !== bairroComCidade) {
                rua = original;
            }
            // Bairro sem prefixo: "Av. X, 50, Centro".
            if (!bairroExtraido) {
                const separador = original.lastIndexOf(",");
                if (separador !== -1 &&
                    normalizarLocalidadePdf(original.slice(separador + 1)) === bairroNormalizado) {
                    rua = original.slice(0, separador).trim();
                } else if (normalizarLocalidadePdf(original).endsWith(" " + bairroNormalizado)) {
                    rua = original.slice(0, original.length - bairroInformado.length).replace(/[\s,;\-–—]+$/, "");
                }
            }
        }

        return { endereco: rua, bairro: bairroInformado || bairroExtraido };
    };

    // Resolve somente campos ausentes; nunca substitui dados já informados
    // pelo cliente. A consulta é apenas para impressão e não grava no banco.
    const consultarCepPdf = async (cep) => {
        ...
        try {
            ...
            console.warn("[Orçamento/PDF] CEP indisponível:", erro.message);
            return null;
        } finally {
            clearTimeout(limite);
        }
    ...
    const dadosCepCliente = (!cliente.cidade || !cliente.bairro || !cliente.estado || !cliente.endereco)
        ? await consultarCepPdf(cliente.cep)
        : null;
    let bairroCadastro = String(cliente.bairro || "").trim();
    // Corrige alguns cadastros importados que juntaram bairro + município.
    // Só dividimos quando o ViaCEP confirma o nome EXATO do bairro restante.
    const cidadeCep = String(dadosCepCliente?.localidade || "").trim();
    const bairroCep = String(dadosCepCliente?.bairro || "").trim();
    if (bairroCadastro && cidadeCep && bairroCep &&
        normalizarLocalidadePdf(bairroCadastro) ===
        normalizarLocalidadePdf(`${bairroCep} ${cidadeCep}`)) {
        bairroCadastro = bairroCep;
    }
    const enderecoBairroCliente = separarEnderecoBairroPdf(
        cliente.endereco || dadosCepCliente?.logradouro,
        bairroCadastro || dadosCepCliente?.bairro,
        cliente.cidade || dadosCepCliente?.localidade
    );
    ...
    const ufCliente = String(cliente.estado || dadosCepCliente?.uf || "").trim().toUpperCase();
    const cidadeBruta = String(cliente.cidade || dadosCepCliente?.localidade || "").trim();
    const cidadeSemUfDuplicada = cidadeBruta.replace(
        new RegExp(`\\s*[-,]\\s*${ufCliente || "(?!)"}$`, "i"), ""
    ).trim();
    const cidadeCliente = [
        cidadeSemUfDuplicada,
        ufCliente
    ]
        .filter(Boolean)
        .join(" - ") || "NÃO INFORMADA — atualizar cadastro do cliente";
    // Uma cidade em branco jamais deve passar despercebida para a impressão.
    if (!cliente.cidade && !dadosCepCliente?.localidade) {
        const imprimirMesmoAssim = window.confirm(
            "A cidade deste cliente não está preenchida e não foi possível recuperá-la pelo CEP. " +
    ...
    const observacoesPdf = String(o.observacoes || "").trim();
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .pagina {
            /* Layout em fluxo: observações longas não podem se sobrepor ao rodapé. */
            min-height: 0;
            position: relative;
            padding-bottom: 0;
        }
        ...
        .observacoes-orcamento h2 { margin-bottom: 2mm; }

        .observacoes-orcamento .texto-observacoes {
        ...
        @media print {
            body { margin: 0; }
            .pagina { min-height: 0; }
            /* O rodapé fica na margem da impressão e não cria uma página vazia
               quando o orçamento inclui observações. */
            .rodape {
                position: fixed;
                left: 0;
                right: 0;
...
<body>
    <div class="pagina">
        ...
        ${observacoesPdf ? `
        <section class="observacoes-orcamento">
            <h2>Observações do orçamento</h2>
            <div class="texto-observacoes">${escapar(observacoesPdf)}</div>
        </section>` : ""}
