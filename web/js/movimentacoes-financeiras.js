...
async function inicializar() {
    preencherFormas(); configurarEventos(); definirPeriodoMes(); await carregarContas(); await carregarDados();
}
...
function abrirModalLancamento(){document.getElementById("formLancamento").reset();document.querySelector('input[name="tipoLancamento"][value="ENTRADA"]').checked=true;document.getElementById("dataLancamento").value=dataInput();document.getElementById("competenciaLancamento").value=dataInput();const padrao=contasFinanceiras.find(c=>c.padrao);if(padrao)document.getElementById("contaLancamento").value=padrao.id;document.getElementById("modalLancamento").classList.add("active");}
function fecharModalLancamento(){document.getElementById("modalLancamento").classList.remove("active");}
async function salvarLancamento(e){e.preventDefault();const b=document.getElementById("btnSalvarLancamento");b.disabled=true;const payload={tipo:document.querySelector('input[name="tipoLancamento"]:checked').value,contaFinanceiraId:Number(document.getElementById("contaLancamento").value),descricao:document.getElementById("descricaoLancamento").value.trim(),valor:Number(document.getElementById("valorLancamento").value),dataMovimentacao:document.getElementById("dataLancamento").value,dataCompetencia:document.getElementById("competenciaLancamento").value||null,formaPagamento:document.getElementById("formaLancamento").value||null,documento:document.getElementById("documentoLancamento").value.trim()||null,observacoes:document.getElementById("observacoesLancamento").value.trim()||null};try{const r=await post("/movimentacoes-financeiras/manual",payload);if(!r?.sucesso)throw new Error(r?.mensagem||"Erro ao lançar movimentação.");fecharModalLancamento();mostrarAviso(r.mensagem||"Lançamento registrado.");await Promise.all([carregarContas(),carregarDados()]);}catch(erro){mostrarAviso(erro.message,"error");}finally{b.disabled=false;}}
