            ...
            include: {
                ...
                integracaoFocusNfe: {
                    select: {
                        id: true,
                        ativo: true,
                        tokenHomologacaoCriptografado: true,
                        tokenProducaoCriptografado: true
                    }
                }
            }
        },
...
class NfeRepository {
    ...
    buscarPorVenda(vendaId, empresaId) {
        return prisma.notaFiscal.findFirst({ where: { vendaId, empresaId } });
    }

    async criarPreparada(venda, snapshot) {
        return prisma.$transaction(async (tx) => {
            const existente = await tx.notaFiscal.findUnique({ where: { vendaId: venda.id } });
            if (existente) return existente;

            const fiscal = await tx.configuracaoFiscal.findUnique({ where: { empresaId: venda.empresaId } });
            if (!fiscal) throw new Error("Configuração fiscal da empresa não encontrada.");
    ...
    atualizar(notaId, dados) {
        return prisma.notaFiscal.update({ where: { id: notaId }, data: dados });
    }
}
