...
async function sincronizarSacMais() {
    const confirmou = confirm(
        "Importar/atualizar os clientes já cadastrados no SacMais?"
    );

    if (!confirmou) return;

    try {
        mostrarMensagem("Sincronizando clientes do SacMais...");

        const resposta = await post("/integracoes/sacmais/clientes/importar", {});

        if (!resposta?.sucesso) {
            throw new Error(resposta?.mensagem || "Não foi possível sincronizar o SacMais.");
        }

        await carregarClientes();

        mostrarMensagem(
            `SacMais sincronizado: ${resposta.importados || 0} importados e ${resposta.atualizados || 0} atualizados.`
        );
    } catch (erro) {
        console.error(erro);
        mostrarMensagem(erro.message || "Erro ao sincronizar SacMais.");
    }
}
